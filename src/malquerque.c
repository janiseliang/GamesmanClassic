...
void GameSpecificMenu()
{
	...
	case 'W':
		...
		intWidth = GetMyUInt();
		if (intWidth != 3) {
			printf("Sorry, only board width 3 is currently supported.\n");
		}
		else if (intWidth % 2 == 1) {
...
/* Helper Functions for Generate Moves
   Lower left of display is (0, 0), top left is arraynum 0. */
...
int canMoveDir(int arraynum, int dir, int current) {
	if (arraynum >= gBoardSize || arraynum < 0)
		return 0; // prevent moving out of board
	...
	if (arraynum % 2 && dir % 2) {
		return 0; // odd arraynum positions don't have diagonals
	}
